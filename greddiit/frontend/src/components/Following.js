...
const Following = ({ following }) => {
  ...
  useEffect(() => {
    ...
    axios.post('http://localhost:5000/following-list', { username: localStorage.getItem("username") })
      .then(response => {
        if (response.data) {
          setData(response.data[0].following);
  ...
  useEffect(() => {
    if (unfollow != "") {
      ...
      axios.post('http://localhost:5000/unfollow', { unfollow: unfollow, username: localStorage.getItem("username") })
        .then(response => {
          if (response.data) {
                    ...
                    <div className="media">
                      ...
                      <div className="media-body">
                        <div className="pull-right">
                          <button type="button" className="btn btn-info btn-sm waves-effect waves-light" onClick={() => setUnfollow(follower)} >UNFOLLOW</button>
                        </div>
